...
KERNEL_FLAGS = -g -Wno-error=incompatible-pointer-types  -Wno-error=int-conversion -fno-stack-protector -m32 -c -ffreestanding -fno-asynchronous-unwind-tables -fno-pie -Wint-conversion $(addprefix -I,$(INCLUDE_DIRS))
KERNEL_OBJECT = $(BUILD_DIR)/kernel.elf
OUTFILE = square-kernel.iso
LINKER = linker.ld
LINKER_FILES = $(BUILD_DIR)/multiboot_header.o $(BUILD_DIR)/starter.o $(BUILD_DIR)/gdt.o $(BUILD_DIR)/idt.o $(KERNEL_OBJECT) $(BUILD_DIR)/vga_buffer.elf $(BUILD_DIR)/liballoc.o $(BUILD_DIR)/paging.o  $(BUILD_DIR)/page_allocator.o $(BUILD_DIR)/string.o $(BUILD_DIR)/MMU.o $(BUILD_DIR)/thread.o
DEBUG_FLAGS = -g
...
build_: $(INIT_KERNEL_FILES) $(BOOTSTRAP_FILE) $(KERNEL_FILES)
	...
	$(CC) $(KERNEL_FLAGS) $(SRC_DIR)/kernel/thread.c -o $(BUILD_DIR)/thread.o
